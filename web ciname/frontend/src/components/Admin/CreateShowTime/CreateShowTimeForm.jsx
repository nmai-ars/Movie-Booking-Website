...
export default function CreateShowTimeForm(props) {
  ...
  const layngayChieu = (event) => {
    // Sửa format thành YYYY-MM-DD để MySQL hiểu được
    let ngayChieu = moment(event.target.value).format("YYYY-MM-DD"); 
  ...
  let [danhSachRap, setDanhSachRap] = useState([]); // State chứa rạp
  // Khi maCumRap thay đổi (người dùng chọn cụm khác), gọi API lấy rạp mới
useEffect(() => {
    if (maCumRap && maCumRap !== '#') {
        qLyPhimService.layThongTinRapTheoCum(maCumRap)
            .then(res => {
                setDanhSachRap(res.data); // Lưu vào state
            })
            .catch(err => console.log(err));
    } else {
        setDanhSachRap([]); // Nếu chưa chọn cụm thì xóa danh sách rạp
    }
}, [maCumRap]);
...
const renderHinhAnhPhim = () => {
    return danhSachPhim.map((phim, index) => {
      if (maPhim === phim.maPhim) {
        // --- [SỬA ĐOẠN NÀY] Xử lý link ảnh ---
        let imgUrl = phim.hinhAnh;
        
        // Kiểm tra nếu chưa có http/https thì nối thêm domain backend
        if (imgUrl && !imgUrl.startsWith("http") && !imgUrl.startsWith("https")) {
          imgUrl = `http://localhost:88/hinhanh/${imgUrl}`;
        }
        // -------------------------------------

        return <img src={imgUrl} alt={phim.tenPhim} key={index} />;
      } else {
        return null;
      }
    });
            ...
            <form className="addshowtime-form">
              <div className="row">
                ...
                <div className="col-md-6 col-sm-12">
                  ...
                  <div className="textb">
                    <input
                      type="time"  // Đổi text thành time
                      step="1"     // Để chọn được cả giây (nếu cần)
                      name="giochieu"
